...
router.put(
  ...
  [
    ...
    [
      ...
      check("feildofzheimer", "Field of Zheimer is required").not().isEmpty(),
      check("from", "From Date is required").not().isEmpty(),
    ],
  ],
  async (req, res) => {
    ...
    try {
      ...
      profile.patient.unshift(newEdu);
...
// @route   DELETE api/profile/patient/:edu_id
// @desc    Delete patient from profile
// @access  Private
router.delete("/patient/:edu_id", auth, async (req, res) => {
  try {
    const profile = await Profile.findOne({ user: req.user.id });
